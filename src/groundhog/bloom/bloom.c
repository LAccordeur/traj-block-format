...
void bloom_filter_rebuild_default(bloom_filter *filter, bit_vect *vect, void* bit_mem, int num_bits) {
    bit_vect_rebuild(vect, bit_mem, num_bits);
    filter->vect = vect;
    filter->num_functions = 2;
    filter->num_items = -1; // not applicable
    filter->hash_functions[0] = djb2;
    filter->hash_functions[1] = sdbm;
}
