...
{
    struct flush_test_sequence	*sequence = arg;
...
static void
reset_zone_and_wait_for_completion(struct flush_test_sequence *sequence)
{
...
static void
init_sequence(struct flush_test_sequence *sequence,  struct ns_entry	*ns_entry) {
...
flush_test(void)
{
    ...
    TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
        ...
        struct flush_test_sequence	sequence;

        init_sequence(&sequence,  ns_entry);

        rc = spdk_nvme_ns_cmd_flush(ns_entry->ns, ns_entry->qpair,
                                    flush_complete, &sequence);
        if (rc != 0) {
