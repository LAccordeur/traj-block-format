...
static void
generate_id_temporal_computation_descriptor(void *buf, struct id_temporal_predicate *predicate, int estimated_result_block_num) {
    struct isp_descriptor desc = {.isp_type = 0, .oid = predicate->oid, .time_min = predicate->time_min, .time_max = predicate->time_max, .lba_count = 0, .estimated_result_page_num = estimated_result_block_num};
...
static void
generate_spatio_temporal_computation_descriptor(void *buf, struct spatio_temporal_range_predicate *predicate, int estimated_result_block_num) {
    struct isp_descriptor desc = {.isp_type = 1, .lon_min = predicate->lon_min, .lon_max = predicate->lon_max, .lat_min = predicate->lat_min, .lat_max = predicate->lat_max, .time_min = predicate->time_min, .time_max = predicate->time_max, .lba_count = 0, .estimated_result_page_num = estimated_result_block_num};
    int desc_size = calculate_isp_descriptor_space(&desc);
    if (desc_size >= 4096) {
        printf("the descriptor size is too big\n");
    }
...
static void
init_sequence(struct hello_world_sequence *sequence, size_t block_size, int estimated_result_block_num, int block_index, struct ns_entry	*ns_entry, size_t *sz, struct id_temporal_predicate *id_predicate, struct spatio_temporal_range_predicate *range_predicate, bool is_id_temporal_query) {
    /*
        ...
        for (int i = 0; i < 1; i++) {
            if (i == 0) {
                ...
                init_sequence(&sequence, read_block_size, estimated_result_block_num, 1, ns_entry, &sz, id_predicates[i], st_predicates[i],
                              is_id_temporal_query);

                clock_t begin1 = clock();
                rc = spdk_nvme_ns_cmd_exe(ns_entry->ns, ns_entry->qpair, sequence.buf,
                                          0, /* LBA start */
...
/*static void
hello_world_single_request(void)
{
    struct ns_entry			*ns_entry;
    int				rc;
    ...
    TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
        *//*
        ...
        int read_block_size = 4096;
        int read_block_num = 300;

        int total_count = 0;

            struct hello_world_sequence	sequence;
            size_t				sz;
            init_sequence(&sequence,  read_block_size, 1, ns_entry, &sz);
